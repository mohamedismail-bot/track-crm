...
function reducer(state: { toasts: ToasterToast[] }, action: Action): { toasts: ToasterToast[] } {
  switch (action.type) {
    ...
    case "DISMISS_TOAST": {
      ...
      if (toastId) return { toasts: state.toasts.map((t) => (t.id === toastId ? { ...t, open: false } : t)) };
      return { toasts: state.toasts.map((t) => ({ ...t, open: false })) };
    }
    case "REMOVE_TOAST": {
      const { toastId } = action;
      if (toastId) return { toasts: state.toasts.filter((t) => t.id !== toastId) };
      return { toasts: [] };
    }
  }
}
...
function toast(props: Omit<Toast, "id">) {
  const id = genId();
  const update = (dirtyProps: Partial<Omit<Toast, "id">>) =>
    dispatch({ type: "UPDATE_TOAST", toast: { ...dirtyProps, id } });
...
export { Toast, ToastClose, ToastTitle, ToastDescription, useToast, toast };