...
**Status: Building. Phases 1–8 shipped (schema+status engine, catalog, order form, approval+deliverable spawn, warehouse label + credit-on-deliver, credit ledger, notifications/dashboard/reports).**
...
7. ~~**Credit ledger**~~ ✅ — accounts, debit on final approval, transaction, `credit.view` routes, Credit page + chip + Settings toggles (round 12).
8. ~~**Notifications + dashboard + reports**~~ ✅ — `GIFT_DELIVERED` notification names the credit effect; `/api/dashboard` gains credit outstanding (top spenders + balances, team-scoped), active exception orders, and a gifting report (order totals + average order value); Reports page renders all three (round 13).
